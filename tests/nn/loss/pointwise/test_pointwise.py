...
class TestPointwiseMSELoss:
    def test_zero_loss(self):
        ...
        loss = PointwiseMSELoss()(logits, labels)
    ...
    def test_reduction_sum(self):
        logits = torch.tensor([[[1.0], [2.0]]])
        labels = torch.tensor([[[3.0], [4.0]]])
        loss = PointwiseMSELoss(reduction="sum")(logits, labels)
        assert loss.item() == pytest.approx(8.0)

    def test_reduction_none(self):
        logits = torch.tensor([[[1.0], [2.0]]])
        labels = torch.tensor([[[3.0], [4.0]]])
        loss = PointwiseMSELoss(reduction="none")(logits, labels)
    ...
    def test_with_item_mask(self):
        ...
        loss = PointwiseMSELoss()(logits, labels, item_mask=mask)
...
class TestPointwiseKLDivergenceLoss:
    def test_perfect_match_low_loss(self):
        ...
        loss = PointwiseKLDivergenceLoss()(logits, labels)
    ...
    def test_reduction_none(self):
        ...
        loss = PointwiseKLDivergenceLoss(reduction="none")(logits, labels)
        assert loss.shape == (1, 2)
    ...
    def test_with_item_mask(self):
        ...
        mask = torch.tensor([[True, True, False]])
        loss_masked = PointwiseKLDivergenceLoss()(logits, labels, item_mask=mask)
        loss_unmasked = PointwiseKLDivergenceLoss()(logits[:, :2], labels[:, :2])
    ...
    def test_unnormalised_labels(self):
        ...
        loss_raw = PointwiseKLDivergenceLoss()(logits, labels_raw)
        loss_norm = PointwiseKLDivergenceLoss()(logits, labels_norm)
